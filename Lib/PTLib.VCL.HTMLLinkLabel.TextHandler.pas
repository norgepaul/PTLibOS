...
unit PTLib.VCL.HTMLLinkLabel.TextHandler;
...
type
  ETextHandlerError = class(EPTLibError);
...
begin
  if not HasNext then
    raise ETextHandlerError.Create('No More Words');
...
begin
  for I := 0 to FList.Count - 1 do
    ...
    if FList[I] is TStringElement then
      ...
      raise ETextHandlerError.Create('Unsupported');
