...
unit PTLib.VCL.HTMLLinkLabel.Tree;
...
uses
  ...
  PTLib.VCL.HTMLLinkLabel.Tools;

type
  ENodeError = class(EPTLibError);
...
begin
  if IsWordInfoInArray(Pos) then
    ...
    raise ENodeError.Create('Word Info Index Out Of Bounds');
end;
...
begin
  ...
  raise ENodeError.Create('Get Node Type Unknown Class');
end;
